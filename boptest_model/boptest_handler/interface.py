...
def check_response(response):
    ...
    if isinstance(response, requests.Response):
        ...
        print("Exiting!")
        sys.exit()
    print("Unexpected error: {}".format(response))
    print("Exiting!")
    sys.exit()
...
def control_test(testcase_name, control_module='', start_time=0, warmup_period=0, length=24*3600, scenario=None, step=300, customized_kpi_config=None, use_forecast=False):
    ...
    try:
        ...
        response = requests.put(
            '{0}/results/{1}'.format(url,testid), 
            json={'point_names': points, 'start_time': start_time, 'final_time': final_time},
            timeout=300  # Add timeout parameter (adjust value as needed)
        )
...
def control_test_with_points(testcase_name, control_module='', start_time=0, warmup_period=0, length=24*3600, scenario=None, step=300, customized_kpi_config=None, use_forecast=False, points=None, url='http://127.0.0.1:80', verbose=False):
    ...
    #assert point is a list
    assert isinstance(points, list), "points must be a list"
    assert len(points) > 0, "points must be provided"
    assert all(isinstance(point, str) for point in points), "all points must be strings"
