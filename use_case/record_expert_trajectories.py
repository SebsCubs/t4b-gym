...
def get_action_from_baseline(model, policy_config):
    # This function should extract the baseline action for the current observation
    # For a true baseline, you may need to call the baseline controller or extract the setpoints from the model
    # Here, we use the model's current setpoints as the baseline action
    action = []
    for component_id, actions in policy_config['actions'].items():
        for action_name, action_config in actions.items():
            # Try to get the value from the model's components
            comp = model.components[component_id]
